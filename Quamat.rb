...
class Quamat < Jacksloon
 ...
 def boxo()
  ...
  for j in 0..3 do #wasz 5/32
   ...
   jamesjohns(@curxo+((10.0-(7*digno*jig))/4),@stary-signo*((3.0/4)-(j/2.0)))
   else jamesjohns(@curxo+(15.0/4),@stary-((3.0/4)-(3/2.0)))
   end
  for i in 0..5 do
    unless j == 3 then
	...
	if j == 3 and (i%2)==0 then
	 jamesjohns(@curxo+(7.0/4)+((i-(@@numero%2))/2.0),@stary-signo*((3.0/4)-(j/2.0)))
	end
   end
  end
  @curxo += @width /2
